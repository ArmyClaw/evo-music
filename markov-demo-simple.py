...
def demo_markov_melody_engine():
    ...
    # 显示前8个音符
    print("   前8个音符:")
    for i, state in enumerate(melody1[:8]):
        print(f"     {i+1}. {state.note.name}{state.octave} - {state.duration:.2f}拍 (力度: {state.velocity})")
    
    demo_files.append("demo_happy_melody.txt")
    ...
    for emotion in emotions:
        ...
        emotion_files.append(f"demo_{emotion}_melody.txt")
    ...
    demo_files.append("demo_custom_melody.txt")
...
def demo_interactive_generation():
    ...
    for style_name, emotion in styles.items():
        ...
        files_created.append(f"demo_{emotion}_short.txt")
    
    return files_created


def save_melody_to_file(melody, filename):
    """将旋律保存到文本文件"""
    content = f"# 旋律数据 - {filename}\n"
    content += f"生成时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n"
    content += f"音符数量: {len(melody)}\n\n"
    
    for i, state in enumerate(melody):
        content += f"{i+1:2d}. {state.note.name}{state.octave} "
        content += f"时长: {state.duration:.2f}拍 "
        content += f"力度: {state.velocity} "
        content += f"MIDI: {state.get_midi_note()}\n"
    
    with open(filename, "w", encoding="utf-8") as f:
        f.write(content)
    
    print(f"   📁 保存旋律: {filename}")
...
def main():
    ...
    try:
        ...
        # 保存示例旋律到文件
        melody1 = generate_melody(emotion="happy", length=16, start_note="C")
        save_melody_to_file(melody1, "demo_happy_melody.txt")
        
        # 显示总结
        print("\n🎉 演示完成！")
        print("=" * 60)
        print(f"📁 总共准备了 {len(all_files)} 个演示文件:")
