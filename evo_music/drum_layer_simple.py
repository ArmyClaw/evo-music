...
try:
    ...
    @dataclass
    class NoteEvent:
        drum_type: Optional['DrumType'] = None
        time: float = 0.0
    ...
    class RhythmFactory:
        @staticmethod
        def create_basic_pattern(time_signature, tempo=120):
            events = [NoteEvent(None, i * 1.0, 80) for i in range(4)]
            return RhythmPattern(f"Basic {time_signature}", time_signature, events, tempo)
...
def test_drum_layer():
    ...
    for style in styles:
        ...
        # 测试输出MIDI事件
        midi_events = drum_pattern.to_midi_events()
        print(f"MIDI事件数: {len(midi_events)}")
